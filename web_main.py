...
async def result_async(request):
	print("Sending final.png to frontend for download")
...
@routes.get("/getoriginalresult/{taskid}")
async def bboxesresultresult_async(request):
	print("Sending input.png to frontend")
	im = Image.open("result/" + request.match_info.get('taskid') + "/input.png")
	stream = BytesIO()
	im.save(stream, "PNG")
	return web.Response(body=stream.getvalue(), content_type='image/png')
...
async def handle_post(request):
    ...
    try:
        ...
        #img.verify() #This closes the file, and breaks code later down the line.
...
async def manual_translate_async(request):
	print('New Manual Request, handling Post')
	x = await handle_post(request)
	print(x)
	if isinstance(x, tuple):
		print("x is instance")
